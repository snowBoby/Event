...
| 能否添加多个 | 不可以 | 不可以 | 可以 | 可以 |
| 绑定 | <input type="button" onclick="showMessage()"/>  | btn.onclick=function(){} | btn.addEventListener( "click",handle,false) | btn.attachEvent( "onclick",handle) |
| 删除 | - | btn.onclick=null | btn.removeEventListener( "click",handle,false)|btn.detachEvent( "onclick",handle) |
